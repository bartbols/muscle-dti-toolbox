...
function [F,V,N] = stlbinary(M)
    ...
    % --- start of edit by Bart Bolsterlee 28/07/2016 --
    % Remove duplicate values from vertices. If a vertex is shared with
    % adjacent faces it only needs to be stored once but all corners of all
    % faces are stored independently now, so there usually is redundancy in
    % the data.
%     
    [V,~,ic] = unique(V,'rows');
    F = ic(F);
