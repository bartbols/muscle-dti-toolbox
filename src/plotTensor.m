function plotTensor(tensor,varargin)
...
addParameter(p,'mask',[])
addParameter(p,'ev1',false) % plot only eigenvector
...
for i=1:n
    ...
    if plot_ev1 == true % plot only primary eigenvector
        if first == 0
            P = NaN(3*n,3);
            switch color
                case  {'ev1','ev2','ev3'}
        ...
        P((3*i)-2,:)           = [Xv(i) Yv(i) Zv(i)]-0.5*ev1*scale;
        P((3*i)-1,:)           = [Xv(i) Yv(i) Zv(i)]+0.5*ev1*scale;
        switch color
            case 'ev1'
                cdata(3*(i-1)+(1:3),1:3) = repmat(abs(ev1),3,1);
            case 'ev2'
                cdata(3*(i-1)+(1:3),1:3) = repmat(abs(ev2),3,1);
            case 'ev3'
                cdata(3*(i-1)+(1:3),1:3) = repmat(abs(ev3),3,1);
            case 'fa'
                cdata(3*(i-1)+(1:3),1) = fa(i);
            case 'az'
                cdata(3*(i-1)+(1:3),1) = atan2(ev1(3)*sign(ev1(1)),ev1(2)*sign(ev1(1)));
        end
        
    else % plot ellipsoid for current tensor
        if first == 0
        ...
        switch color
            case 'ev1'
                ...
                cdata((1:nv) + (i-1)*nv,:)    = repmat(abs(ev1),nv,1);
            case 'ev2'
                %       Color the ellipsoid with the direction of the secondary eigenvector.
                cdata((1:nv) + (i-1)*nv,:)    = repmat(abs(ev2),nv,1);
            case 'ev3'
                %       Color the ellipsoid with the direction of the secondary eigenvector.
                cdata((1:nv) + (i-1)*nv,:)    = repmat(abs(ev3),nv,1);
            case 'fa'
                % Color according to fractional anisotropy
                cdata((1:nv) + (i-1)*nv)      = fa(i);
            case 'az'
                % Azimuthal angle
                cdata((1:nv) + (i-1)*nv)      = atan2(ev1(3)*sign(ev1(1)),ev1(2)*sign(ev1(1)));
        end
        face_offset                   = face_offset + nv;
    end
end

if plot_ev1 == true  % plot only primary eigenvector lines
