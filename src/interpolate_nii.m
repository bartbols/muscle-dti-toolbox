function [ values ] = interpolate_nii( nii,points )
...
if numel(imdim) == 3
    values = interp3(I,voxel_coords(:,2)+1,voxel_coords(:,1)+1,voxel_coords(:,3)+1);
elseif numel(imdim) == 4
    values = zeros(size(points,1),imdim(4));
    for i = 1 : imdim(4)
        values(:,i) = interp3(I(:,:,:,i),voxel_coords(:,2)+1,voxel_coords(:,1)+1,voxel_coords(:,3)+1);
    end
else
