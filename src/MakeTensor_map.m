function varargout = MakeTensor_map( fib_filename,DTI_filename, fa_threshold,varargin )
%MAKETENSOR_MAP Reads the tensor components from the DSI-reconstructed
...
%
% ----------------- USAGE ----------------- 
% Tensor_filename = MakeTensor_map( fib_filename,DTI_filename, fa_threshold)
% or 
% Tensor_filename = MakeTensor_map( fib_filename,DTI_filename, fa_threshold,Tensor_filename)
%
% ----------------- INPUT ----------------- 
...
addOptional(p,'Tensor_filename',[DTI_filename(1:end-7) '_Tensor.nii.gz'],@(x) contains(x,'.nii'))
